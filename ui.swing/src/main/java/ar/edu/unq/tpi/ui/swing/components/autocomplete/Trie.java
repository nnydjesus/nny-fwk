...
public class Trie {
     ...
     */
    public Trie(final boolean ignoreCase) {
     ...
     */
    private final int match(final String a, final int startOffset, final int stopOffset, final String b) {
     ...
     * @modifies this
     */
    public Object add(String key, final Object value) {
     ...
     */
    private TrieNode fetch(final String prefix) {
     ...
     */
    public Iterator getPrefixedBy(final String prefix, final int startOffset, final int stopOffset) {
    ...
    private class ValueIterator extends NodeIterator {
        ValueIterator(final TrieNode start) {
            super(start, false);
        }
     ...
     */
    // public void trim(final Function valueCompactor) throws
    // IllegalArgumentException, ClassCastException {
    // if (valueCompactor != null) {
    // // For each node in this...
    // for (Iterator iter = new NodeIterator(root, true); iter.hasNext();) {
    ...
    public class NodeIterator extends UnmodifiableIterator {
         ...
         */
        public NodeIterator(final TrieNode start, final boolean withNulls) {
    ...
    public String toString() {
        ...
        buf.append("<root>");
        this.toStringHelper(root, buf, 1);
        return buf.toString();
    }
     ...
     */
    private void toStringHelper(final TrieNode start, final StringBuffer buf, final int indent) {
        // Print value of node.
...
final class TrieNode {
     ...
     */
    public TrieNode(final Object value) {
        this.value = value;
    }
     ...
     */
    private final TrieEdge get(final int i) {
     ...
     * match, insert at 2).
     */
    private final int search(final char c, final boolean exact) {
     ...
     */
    public TrieEdge get(final char labelStart) {
     ...
     * @modifies this
     */
    public void put(final String label, final TrieNode child) {
        char labelStart;
        int i;
        ...
        if ((i = this.search(labelStart = label.charAt(0), // find closest match
                false)) >= 0) {
     ...
     */
    public boolean remove(final char labelStart) {
        int i;
        if ((i = this.search(labelStart, true)) < 0)
            return false;
    ...
    private class ChildrenForwardIterator extends UnmodifiableIterator {
        ...
        public Object next() {
            if (i < children.size())
                return TrieNode.this.get(i++).getChild();
            throw new NoSuchElementException();
        }
    }
    ...
    private class LabelForwardIterator extends UnmodifiableIterator {
        ...
        public Object next() {
            if (i < children.size())
                return TrieNode.this.get(i++).getLabel();
            throw new NoSuchElementException();
        }
    }
...
final class TrieEdge {
     ...
     * @requires child != null
     */
    TrieEdge(final String label, final TrieNode child) {
        this.label = label;
        this.child = child;
    }
