...
import ar.edu.unq.tpi.commons.configuration.dependency.DependencyBinder;
import ar.edu.unq.tpi.util.commons.exeption.UserException;

@SuppressWarnings({"unchecked","rawtypes"})
public class ServiceLocatorStrategyImpl implements ServiceLocatorStrategy {
	...
	public <T> T locate(final Class<T> serviceClass, final boolean searchAmongNotFinishedOnes) {
        if ( serviceClass == null ) {
            throw new UserException("Service class must be specified");
        }
    ...
    private <T> T instanciateService(final Class<T> serviceClass) {
        try {
           // validateService(serviceClass);
            ...
            throw new UserException("Cannot instanciate service of class " + serviceClass.getName(), e);
        }
    }
        ...
        for ( final Method method : methods ) {
            ...
            if ( methodSignatures.containsKey(name) ) {
                ...
                if ( parametersQtyCollection.contains(parameterQty) ) {
                    throw new UserException("Cannot user parameter type overload in service methods. Service [" + serviceClass.getName() + "] method [" + name + "]");
                }
